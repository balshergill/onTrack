...
exports.displayMinsLate = (liveResult, req) => {
  ...
  for (let i = 0; i < liveResult.length; i++) {
    ...
    const qString = `SELECT ROUND(AVG(COALESCE(dep_minutes_late, 0)),2) + ROUND(AVG(COALESCE(cancelled, 0)),2) AS dep_minutes_late FROM historical_services WHERE station_from='${stationFrom}' AND station_to='${stationTo}' AND sch_dep_time='${depTime}';`;
